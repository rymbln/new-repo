...
format: 
  dashboard:
    output-file: "index"
    # Список тем оформления https://quarto.org/docs/dashboards/theming.html
    theme:
    - cosmo
    # - sketchy
    # scrolling: true
    logo: logo.png
    # Список доступных иконок bootstrap https://icons.getbootstrap.com/
    nav-buttons: 
    - icon: github
...
library(ggplot2)
library(ggrepel)
# Установим тему шрифтов по умолчанию
...
<!-- https://quarto.org/docs/dashboards/inputs.html#sidebars -->
## {.sidebar}

Боковая панель

В ней можно располагать необязательную информацию:

Данное исследование рассматривало  `{r} nrow(pat)` пациентов из `{r} length(unique(pat$CITYNAME))` городов России и Беларуси

|               |                                  |
|-------------- |----------------------------------|
| **Центров **  | `{r} length(unique(pat$CENTER))` |
| **Пациентов** | `{r} nrow(pat)`                  |
| **Дата**      | `{r} Sys.Date()`                 |

::: {.callout-note collapse="true"}
### О проекте

Это демонстрационное исследование,
описание доступно [по ссылке](https://ya.ru).
:::
...
<!-- https://quarto.org/docs/dashboards/data-display.html#plots -->
...
<!-- https://quarto.org/docs/dashboards/layout.html#tabsets -->
#### Row {.tabset}
...
<!-- https://quarto.org/docs/dashboards/data-display.html#kable -->
...
<!-- https://quarto.org/docs/dashboards/data-display.html#dt -->
